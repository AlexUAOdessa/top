"""
НЕОНОВЫЕ ФРАКТАЛЫ v2 — ДЕРЕВО С ПАПОРОТНИКАМИ + УЛУЧШЕННЫЙ BLOOM
-----------------------------------------------------------------

Изменения:
• Больше ветвлений (глубина до 13–14)
• Более живые / хаотичные углы разветвления (50–70° + шум)
• Цвет папоротников зависит от уровня глубины (hue shift)
• Настоящий bloom/glow: bright pass + multi-pass Gaussian blur + additive

Установка (если ещё не):
    pip install numba numpy imageio[ffmpeg]

Для теста уменьшите: WIDTH=1280, HEIGHT=720, FRAMES=120, BLOOM_PASSES=2
"""
...
TREE_DEPTH         = 13               # было 11 → больше ветвей
FERN_POINTS        = 10000            # можно уменьшить до 6000–8000
FERN_ON_BRANCH_LEN = 8.0

# Bloom / Glow настройки
BLOOM_THRESHOLD    = 0.75             # от какой яркости начинать glow
BLOOM_INTENSITY    = 1.4              # сила свечения
BLOOM_PASSES       = 3                # 2–4; больше = мягче и медленнее
BLOOM_SIGMA_BASE   = 6.0              # базовый радиус размытия
...
def apply_gaussian_blur(img_in, img_out, sigma):
    # Простой сепарабельный Gaussian blur (горизонталь + вертикаль)
    # Для скорости — фиксированный kernel 9–13 пикселей
...
def apply_bloom(img, temp1, temp2):
    ...
    # Bright pass + нормализация в float
    for y in nb.prange(h):
        for x in range(w):
            brightness = max(img[y,x,0], img[y,x,1], img[y,x,2]) / 255.0
            if brightness > BLOOM_THRESHOLD:
                mul = (brightness - BLOOM_THRESHOLD) / (1.0 - BLOOM_THRESHOLD)
                mul = mul * mul  # quadratic для более резкого отсечения
                temp1[y,x,0] = img[y,x,0] * mul / 255.0
                temp1[y,x,1] = img[y,x,1] * mul / 255.0
                temp1[y,x,2] = img[y,x,2] * mul / 255.0
            else:
                temp1[y,x,0] = temp1[y,x,1] = temp1[y,x,2] = 0.0
    
    # Несколько проходов blur с увеличивающимся sigma
    sigma = BLOOM_SIGMA_BASE
    apply_gaussian_blur(temp1, temp2, sigma)
    for p in range(1, BLOOM_PASSES):
        sigma *= 1.6
        apply_gaussian_blur(temp2, temp1, sigma)
        apply_gaussian_blur(temp1, temp2, sigma)
    
    # Добавляем bloom обратно (в оригинал)
    for y in nb.prange(h):
        for x in range(w):
            img[y,x,0] = min(255, img[y,x,0] + temp2[y,x,0] * 255 * BLOOM_INTENSITY)
            img[y,x,1] = min(255, img[y,x,1] + temp2[y,x,1] * 255 * BLOOM_INTENSITY)
            img[y,x,2] = min(255, img[y,x,2] + temp2[y,x,2] * 255 * BLOOM_INTENSITY)
...
def render_tree_fern(frame_idx, w, h, img, temp1, temp2):
    ...
    # Стек для ветвей (увеличил размер стека)
    MAX_STACK = 1024
    ...
    while sp > 0:
        ...
        # Цвет ветви — чуть теплее с глубиной
        base_hue = 0.28 + depth * 0.03
        col_r, col_g, col_b = hsv_to_rgb(base_hue, 0.85, 0.9 + 0.1 * math.sin(frame_idx*0.04 + depth))
        col_r = int(col_r * 255); col_g = int(col_g * 255); col_b = int(col_b * 255)
        
        # Bresenham-like линия с толщиной
        dx = sx2 - sx1; dy = sy2 - sy1
        steps = max(abs(dx), abs(dy), 1)
        xinc = dx / steps; yinc = dy / steps
        x = float(sx1); y = float(sy1)
        ...
        # Папоротник на тонких ветвях
        if blen < FERN_ON_BRANCH_LEN:
            fx, fy, fz = x2, y2, z2
            scale_fern = blen * 0.55
            
            # Цвет папоротника по глубине (hue shift)
        ...
        if depth + 1 < TREE_DEPTH:
            # Более широкий диапазон углов + шум
            ...
            if sp + 2 < MAX_STACK:
                ...
                len_orig = math.sqrt(dx*dx + dy*dy + dz*dz)
                if len_orig > 0.1:
...
def render_and_save(filename):
    ...
    writer = imageio.get_writer(
        filename, fps=FPS, codec='libx264', quality=9,
    ...
    for f in range(FRAMES):
        ...
        # Применяем bloom только если включено
        if BLOOM_PASSES > 0:
...
print("Запуск рендера улучшенного дерева с папоротниками + bloom")
render_and_save("neon_tree_fern_bloom_v2_4k_30s.mp4")
