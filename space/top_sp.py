...
COLORS = [
    ...
    (200, 100, 0), (0, 100, 200), (100, 0, 200)
]

def get_flag(country_name):
    # Проверка исключения для СССР
    if EXCEPTION_FLAGS.get(country_name) == "red_rect":
        flag = Image.new("RGBA", (60, 40), (255, 0, 0, 255))
        return flag
...
def generate_frame(filename, current_year, state, global_max):
    ...
    for i, (name, _, val, _) in enumerate(sorted_state):
        ...
        # Название страны (ручное выравнивание)
...
def main():
    ...
    for y in data_by_year:
        for c in data_by_year[y]: all_countries.add(c)
    ...
    global_max = max(max(t.values()) for t in totals_history.values())
    ...
    for i in range(len(years) - 1):
        ...
        for f in range(FRAMES_PER_PERIOD):
            ...
            state = [(n, 0, totals_history[y1][n]*(1-prog) + totals_history[y2][n]*prog, 0) for n in all_countries]
            generate_frame(f"frame_{frame_idx:05d}.png", y1 + (y2-y1)*prog, state, global_max)
            frame_idx += 1

    last_year = years[-1]
    final_state = [(n, 0, totals_history[last_year][n], 0) for n in all_countries]
    for i in range(int(FPS * FINAL_FREEZE_SECONDS)):
        generate_frame(f"frame_{frame_idx:05d}.png", last_year, final_state, global_max)
        frame_idx += 1

    subprocess.run(["ffmpeg", "-y", "-framerate", str(FPS), "-i", os.path.join(TEMP_DIR, "frame_%05d.png"), "-c:v", "libx264", "-pix_fmt", "yuv420p", OUTPUT_VIDEO])
