...
# Глобальная сетка для растровых задач
_x = np.linspace(-2.2, 2.2, GRID_RES)
_y = np.linspace(-2.2, 2.2, GRID_RES)
...
# --- БЛОК 1: 10 РАСТРОВЫХ ЯДЕР (SURFACES) ---

@jit(nopython=True, parallel=True)
def calc_mandel(f, X, Y):
...
def calc_ship(f, X, Y):
    ...
    for i in prange(GRID_RES):
        for j in range(GRID_RES):
            x0, y0 = X[i,j]/zoom - 1.75, Y[i,j]/zoom
            zx, zy, it = 0.0, 0.0, 0
            while zx*zx + zy*zy <= 4 and it < MAX_ITER:
...
@jit(nopython=True)
def calc_plasma(f, X, Y):
    t = f * 0.1
    return (np.sin(X+t) + np.sin((Y+t)/2) + np.cos(np.sqrt(X**2+Y**2)+t)) * 5 + 10

@jit(nopython=True, parallel=True)
def calc_newton(f, X, Y):
    Z = np.zeros((GRID_RES, GRID_RES))
    zoom = 1.0 + (f / 40)
    for i in prange(GRID_RES):
        for j in range(GRID_RES):
            zx, zy = X[i,j]/zoom, Y[i,j]/zoom
            it = 0
            for _ in range(30):
                r2 = zx*zx + zy*zy
                if r2 < 1e-6: break
                dist2 = r2*r2
                nx = 2/3 * zx + (zx*zx - zy*zy) / (3 * dist2)
                ny = 2/3 * zy - 2*zx*zy / (3 * dist2)
                if (nx-zx)**2 + (ny-zy)**2 < 1e-4: break
                zx, zy, it = nx, ny, it + 1
            Z[i,j] = it + np.sqrt(zx*zx + zy*zy)
    return Z

@jit(nopython=True, parallel=True)
def calc_biomorph(f, X, Y):
    Z = np.zeros((GRID_RES, GRID_RES))
    cr, ci = 0.5 + 0.1*np.sin(f/50), 0.5
    for i in prange(GRID_RES):
        for j in range(GRID_RES):
            zx, zy, it = X[i,j], Y[i,j], 0
            while zx*zx + zy*zy < 10 and it < 20:
                x2, y2 = zx*zx, zy*zy
                x4, y4 = x2*x2, y2*y2
                nx = zx*(x4 - 10*x2*y2 + 5*y4) + cr
                ny = zy*(5*x4 - 10*x2*y2 + y4) + ci
                zx, zy = nx, ny
                if abs(zx) < 2 or abs(zy) < 2:
                    Z[i,j] = it * 5
                    break
                it += 1
    return Z
...
@jit(nopython=True, parallel=True)
def calc_phoenix(f, X, Y):
    Z = np.zeros((GRID_RES, GRID_RES))
    c, p = -0.4, 0.3
    for i in prange(GRID_RES):
        for j in range(GRID_RES):
            zx, zy, zpx, zpy, it = Y[i,j], X[i,j], 0.0, 0.0, 0
            while zx*zx + zy*zy < 4 and it < 50:
                nx = zx*zx - zy*zy + c + p*zpx
                ny = 2*zx*zy + p*zpy
                zpx, zpy, zx, zy, it = zx, zy, nx, ny, it + 1
            Z[i,j] = it
    return Z

@jit(nopython=True, parallel=True)
def calc_lyapunov(f, X, Y):
    Z = np.zeros((GRID_RES, GRID_RES))
    for i in prange(GRID_RES):
        for j in range(GRID_RES):
            a, b, cur, lyap = X[i,j]+3.5, Y[i,j]+3.5, 0.5, 0.0
            for it in range(40):
                r = a if it%2==0 else b
                cur = r*cur*(1-cur)
                if cur <= 0 or cur >= 1: break
                lyap += np.log(abs(r*(1-2*cur)))
            Z[i,j] = max(lyap, -50)
    return Z

@jit(nopython=True, parallel=True)
def calc_clifford(f, X, Y):
    ...
    return np.log1p(res)*8

@jit(nopython=True, parallel=True)
def calc_dejong(f, X, Y):
    res = np.zeros((GRID_RES, GRID_RES))
...
# --- БЛОК 2: 10 ВЕКТОРНЫХ ФУНКЦИЙ (VOLUMETRIC) ---
...
def draw_lorenz(ax, f):
    ...
    for _ in range(2500 + f*10):
        ...
        x+=dx; y+=dy; z+=dz
        pts.append([x, y, z])
    p = np.array(pts); ax.plot(p[:,0], p[:,1], p[:,2], color='#ff4400', lw=0.8)

def draw_dragon(ax, f):
    it, am = min(12, int(f/70)+6), np.deg2rad(90 + np.sin(f/45)*15)
    pts = np.array([[0,0], [1,0]], dtype=np.float64)
    for _ in range(it):
        c = pts[-1]; rot = np.array([[np.cos(am), -np.sin(am)], [np.sin(am), np.cos(am)]])
...
def draw_fern(ax, f):
    ...
    p = np.array(pts); ax.scatter(p[:,0], p[:,1], p[:,2], s=0.2, color='#00ff66', alpha=0.3)

def draw_levy(ax, f):
    def lv(p1, p2, d):
        if d==0: return [p1, p2]
        v = p2-p1; p3 = p1 + 0.5*np.array([v[0]-v[1], v[0]+v[1]])
...
def draw_henon(ax, f):
    ...
    pts = [[x, y, 0]]
    for i in range(n):
        x, y = 1 - a*x**2 + y, b*x; pts.append([x, y, i/2000])
    p = np.array(pts); ax.scatter(p[:,0], p[:,1], p[:,2], s=0.1, color='#ffcc00', alpha=0.4)

def draw_bifur(ax, f):
    r = np.linspace(3.5, 4.0, 500); x = 0.5*np.ones_like(r)
    for _ in range(100): x = r*x*(1-x)
    for i in range(40):
        x = r*x*(1-x); ax.plot(r, x, i/10, ',w', color=neon_cmap(0.5), alpha=0.3)

def draw_sierp(ax, f):
    pts = np.array([[0,0,0], [1,0,0], [0.5, 0.86, 0], [0.5, 0.4, 0.8]])
    cur = np.array([0,0,0], dtype=np.float64); res = []
    for _ in range(8000):
        cur = (cur + pts[np.random.randint(4)])/2; res.append(cur.copy())
    p = np.array(res); ax.scatter(p[:,0], p[:,1], p[:,2], s=0.1, color='#ffffff')

def draw_lissa(ax, f):
    t = np.linspace(0, 2*np.pi, 2000)
    ax.plot(np.sin(3*t+f/20), np.cos(2*t), np.sin(5*t+f/50), color='#ff00ff', lw=2)

def draw_vicsek(ax, f):
    def vs(x, y, z, s, d):
        if d==0: ax.scatter([x], [y], [z], s=1, color='#00ffff'); return
        ns = s/3
    ...
    for t in tasks:
        ...
        if t['m'] == "s":
            def update(f):
                ...
                Z = t['f'](f, X_GRID, Y_GRID)
                surf = ax.plot_surface(X_GRID, Y_GRID, Z, cmap=t['c'], linewidth=0, antialiased=False, shade=True)
        ...
        send_email('admin@obzhora.org', f'Готов: {t["n"]}', f'Файл успешно создан.')

    send_email('admin@obzhora.org', 'Генерация завершена', 'Все 20 роликов созданы.')